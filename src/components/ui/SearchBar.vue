<template>
  <div class="relative">
    ...
    <div class="absolute left-0 top-full w-full bg-gray-100 rounded-xl z-50 shadow-sm shadow-gray-200 overflow-hidden">
      <template v-if="citiesSuggestions.length !== 0 && !commonStore.isLoading">
      ...
      <div
        v-if="commonStore.isLoading && searchQuery.length !== 0"
...
watchDebounced(searchQuery, async () => {
  await getCitiesSuggestions(searchQuery.value)
    .then((data) => {
      if (data.length > 0) {
